...
class Rocket:
    ...
    def_fire_points = [Point(-5, 12), Point(-3, 14), Point(0, 17), Point(3,14), Point(5,12)]
    ...
    def __init__(self, x, y):
        ...
        for i in self.def_fire_points:
            self.fire_points.append(Point(i.x + self.position.x, i.y + self.position.y))
    ...
    def rotate(self, angle):
        ...
        # https://en.wikipedia.org/wiki/Rotation_matrix
        c_temp = copy.deepcopy(self.def_points) # deepcopy necessary
        f_temp = copy.deepcopy(self.def_fire_points) # deepcopy necessary

        c_points = list()
        for i in c_temp:
            i.rotate_vector(self.angle)
            c_points.append(Point(i.x + self.position.x, i.y + self.position.y))

        f_points = list()
        for i in f_temp:
            i.rotate_vector(self.angle)
            f_points.append(Point(i.x + self.position.x, i.y + self.position.y))

        self.col_points = list(c_points)
        self.fire_points = list(f_points)
    ...
    def draw_fire(self):
        ...
        for i in self.fire_points:
            temp.append(i.returnl())
        pygame.draw.polygon(screen, Colors.red2, temp)
